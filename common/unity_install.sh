...
if device_check "herolte" || device_check "hero2lte" || device_check "gracelte" || device_check "gracerlte"; then
  ui_print " "
  ui_print "- Select Option -"
  ui_print "  [?] Samsung Galaxy S7 / S7 Edge / Note FE Detected"
  ui_print "  [?] Do you want to load the following settings?"
  ui_print "      (Recommended)"
  ui_print ""
  if version_check "28"; then
    ui_print "    -     libexynoscamera.so mod: Enabled (Note FE Pie)"
    MODLIB=S7PIE
  else
    MODLIB=S7
    ui_print "    -     libexynoscamera.so mod: Enabled (S7 Oreo)"
  fi
...
if device_check "dreamlte" || device_check "dream2lte"; then
  ...
  ui_print ""
  if version_check "28"; then
    ui_print "    -     libexynoscamera.so mod: Enabled (S8 Pie)"
    MODLIB=S8PIE
  else
    MODLIB=S8
    ui_print "    -     libexynoscamera.so mod: Enabled (S8 Oreo)"
  fi
...
if device_check "greatlte" || device_check "greatqltezc" || device_check "greatqltechn"; then
  ...
  ui_print ""
  if version_check "28"; then
    ui_print "    -     libexynoscamera.so mod: Enabled (N8 Pie)"
    MODLIB=N8PIE
  else
    MODLIB=N8
    ui_print "    -     libexynoscamera.so mod: Enabled (N8 Oreo)"
  fi
  ui_print "    -  18.5:9 resolution support: Enabled"
...
if [ "$MODLIB" != "none" ]; then
  ...
  cp -f "$TMPDIR/device_specific/$MODLIB/system/lib/libexynoscamera.so"  "$TMPDIR/system/lib/libexynoscamera.so"
else
...
if $SEIGHTSELFIE; then
  ...
  echo '  <local name="FRONT_CAMERA_RESOLUTION_16BY9_LARGE" value="3264x1836" />
    ...
    <local name="FRONT_CAMCORDER_RESOLUTION_640X480" value="true" hdr="true" preview-size="1440x1080" snapshot-support="true"  snapshot-size="3264x2448" vdis="false" effect="true" object-tracking="false" seamless-zoom-support="false"/>' >> $TMPDIR/system/cameradata/camera-feature-v7.xml
else
  ...
  echo '    <local name="FRONT_CAMERA_RESOLUTION_16BY9_LARGE" value="2592x1458" />
    ...
    <local name="FRONT_CAMCORDER_RESOLUTION_640X480" value="true" hdr="true" preview-size="1440x1080" snapshot-support="true"  snapshot-size="2592x1944" vdis="false" effect="true" object-tracking="false" seamless-zoom-support="false"/>' >> $TMPDIR/system/cameradata/camera-feature-v7.xml
fi
